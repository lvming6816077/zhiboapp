...
//            
//            UIImage *saveImage = scrollView.photoImageView.image;
//            UIImage *logoImage = [saveImage imageWithLogo:[UIImage imageNamed:@"tribe_watermark.png"]];
//            
//            [self saveToAlbumWithMetadata:nil imageData:UIImageJPEGRepresentation(logoImage, 1) customAlbumName:@"兴趣部落" completionBlock:^{
//                IDMZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
//                [TBAlertView showCenterAlertView:TBAlertViewTypeSuccess content:@"保存到相册成功！" view:scrollView];
//            } failureBlock:^(NSError *error) {
//                IDMZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
//                [TBAlertView showCenterAlertView:TBAlertViewTypeWarn content:[error localizedDescription] view:scrollView];
//            }];
//        }
...
//    if (error == nil)
//    {
//        [TBAlertView showCenterAlertView:TBAlertViewTypeSuccess content:@"保存到系统相册成功！" view:scrollView];
//    }
//    else
//    {
//        [TBAlertView showCenterAlertView:TBAlertViewTypeWarn content:[error localizedDescription] view:scrollView];
...
//            finalY = -scrollView.size.height;
//        }
//        else {
//            finalY = 2*scrollView.size.height;
//        }
        
        [UIView animateWithDuration:_animationDuration delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            scrollView.center = CGPointMake(scrollView.center.x, finalY);
...
- (void)doubleClickGestureRecognized:(id)sender
{
    IDMZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
    
    CGFloat minZoomScale = scrollView.minimumZoomScale;
    CGFloat maxZoomScale = scrollView.maximumZoomScale;
    CGFloat zoomScale = scrollView.zoomScale;
    
    /**不知道为什么第三方库里面设置最大放大范围，有的图片放大范围很小，所以先这样处理一下，强制让图片至少可以放大2倍*/
    if (zoomScale == minZoomScale && maxZoomScale < 2 * minZoomScale)
    {
        maxZoomScale = 2 * minZoomScale;
        scrollView.maximumZoomScale = 2 * minZoomScale;
    }
    
    zoomScale = (zoomScale != minZoomScale) ? minZoomScale : maxZoomScale;
    
    scrollView.maximumZoomScale = minZoomScale * 2;
...
//    fadeLayer.frame = CGRectMake(0, 0, screenWidth, screenHeight);
//    fadeLayer.backgroundColor = [UIColor clearColor].CGColor;
//    [_applicationWindow.layer addSublayer:fadeLayer];
//    
//    CALayer *resizableLayer = [CALayer layer];
//    resizableLayer.contents = (__bridge id)imageFromView.CGImage;
//    resizableLayer.frame = _senderViewOriginalFrame;
//    resizableLayer.masksToBounds = YES;
//    resizableLayer.contentsGravity = kCAGravityResizeAspectFill;
//    resizableLayer.backgroundColor = [UIColor colorWithWhite:(_useWhiteBackgroundColor) ? 1 : 0 alpha:1].CGColor;
//    [_applicationWindow.layer addSublayer:resizableLayer];
...
//        [resizableLayer removeFromSuperlayer];
    };
    ...
    // 如果高宽比大等于2，则认为是长图，将其初始位置设为从顶端浏览
    if (imageFromView.size.height >= imageFromView.size.width * 2) {
        finalImageViewFrame = CGRectMake(0, 0, screenWidth, imageFromView.size.height / scaleFactor);
    }
    else {//不是长图，居中显示
...
//        colorAnimation.toValue = self.useWhiteBackgroundColor ? (__bridge id)[UIColor whiteColor].CGColor : (__bridge id)[UIColor blackColor].CGColor;
//        colorAnimation.duration = _animationDuration;
//        [fadeLayer addAnimation:colorAnimation forKey:@"color"];
//        
//        CABasicAnimation * baseAnimation = [CABasicAnimation animationWithKeyPath:@"position"];
//        
//        baseAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
//        baseAnimation.fromValue = [NSValue valueWithCGPoint:resizableLayer.position] ;
//        baseAnimation.toValue = [NSValue valueWithCGPoint:CGPointMake(CGRectGetMidX(finalImageViewFrame), CGRectGetMidY(finalImageViewFrame))] ;
//        
//        CABasicAnimation * boundsAnimation = [CABasicAnimation animationWithKeyPath:@"bounds"];
//        
//        boundsAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
//        boundsAnimation.fromValue = [NSValue valueWithCGRect:resizableLayer.bounds] ;
//        boundsAnimation.toValue = [NSValue valueWithCGRect:CGRectMake(0, 0, finalImageViewFrame.size.width, finalImageViewFrame.size.height)] ;
//        
//        CAAnimationGroup * group =[CAAnimationGroup animation];
//        group.removedOnCompletion=NO;
//        group.fillMode=kCAFillModeForwards;
//        group.animations = @[baseAnimation, boundsAnimation];
//        group.duration = _animationDuration;
//        group.delegate = self;
//        [group setValue:completion forKey:@"completion"];
//        [resizableLayer addAnimation:group forKey:@"frame"];
    }
    
}

//- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
//    void (^completion)() = [anim valueForKey:@"completion"];
//    completion();
//}
...
- (void)viewDidLoad {
    ...
    UITapGestureRecognizer *doubleClickGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(doubleClickGestureRecognized:)];
    ...
    [tapGesture requireGestureRecognizerToFail:doubleClickGesture];
...
//    reboot 注释掉下面的代码，因为无需隐藏状态栏，否则导致window 整体位移
...
- (void)updateToolbar {
    // Counter
    //neilliang修改，这里要求只有1张图片也要显示张数
	if ([self numberOfPhotos] >= 1) {
        _counterLabel.text = [NSString stringWithFormat:@"%lu", (unsigned long)(_currentPageIndex+1)];
        _counterLabel.text = [_counterLabel.text stringByAppendingString:@" / "];
        _counterLabel.text = [_counterLabel.text stringByAppendingFormat:@"%lu", (unsigned long)[self numberOfPhotos]];
	} else {
...
{
    /**
     *π计划相册不存在就新建一个相册，若存在，直接把照片存进去。
     */
    [self createAlbumInPhoneAlbum];
    __weak ALAssetsLibrary *assetsLibrary = _assetsLibrary;
