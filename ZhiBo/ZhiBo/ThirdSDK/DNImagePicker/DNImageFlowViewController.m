...
- (void)viewDidLoad {
    [super viewDidLoad];
//    _selectedAssetsArray = [self.delegate hasSelectedAssets];
    [self setupView];
    [self setupData];
}
...
{
    for (NSObject *asset in self.selectedAssetsArray) {
        NSURL *assetURL = nil;
        if ([asset isKindOfClass:[ALAsset class]]) {
            ALAsset *_asset = (ALAsset*) asset;
            assetURL = [_asset valueForProperty:ALAssetPropertyAssetURL];
        } else {
            DNAsset *_dnasset = (DNAsset*) asset;
            assetURL = _dnasset.url;
        }
...
- (void)sendImages
{
    ...
    if (imagePicker && [imagePicker.imagePickerDelegate respondsToSelector:@selector(dnImagePickerController:sendImages:isFullImage:andAssets:)]) {
        [imagePicker.imagePickerDelegate dnImagePickerController:imagePicker sendImages:[self seletedDNAssetArray] isFullImage:self.isFullImage andAssets:[self.selectedAssetsArray mutableCopy]];
    }
